...
class category {
    ...
    public function getAll() {
        $stmt = $this->conn->prepare("SELECT * FROM {$this->table} ORDER BY category_id DESC");
        $stmt->execute();
    ...
    public function getPaginated($page = 1, $perPage = 10) {
        ...
        $stmt = $this->conn->prepare("SELECT c.category_id, c.name, c.description
            FROM {$this->table} c
    ...
    public function delete($id) {
        $stmt = $this->conn->prepare("DELETE FROM {$this->table} WHERE category_id = :id");
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        return $stmt->execute();
    }
}
